...
def extract_prototypes_to_excel():
    """
    读取 object_prototypes.json 文件，提取建筑、岩石、作物、草和树木的信息，
    并将其分别保存到 Excel 文件的不同 Sheet 中。
    """
    try:
        # 定义文件路径
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        json_path = os.path.join(base_dir, 'data_cn', 'object_prototypes.json')
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            # 1. 处理建筑 (building)
            building_data = []
            for building_id, details in data.get('building', {}).items():
            ...
            # 2. 处理岩石 (rock)
            rock_data = []
            for rock_id, details in data.get('rock', {}).items():
            ...
            # 3. 处理作物 (crop)
            crop_data = []
            for crop_id, details in data.get('crop', {}).items():
            ...
            # 4. 处理草 (grass)
            grass_data = []
            for grass_id, details in data.get('grass', {}).items():
                drops = details.get('drops', [])
            ...
            # 5. 处理树 (tree)
            tree_data = []
            for tree_id, details in data.get('tree', {}).items():
        ...
        print(f"❌ 错误: JSON 文件格式无效 at {json_path}")
